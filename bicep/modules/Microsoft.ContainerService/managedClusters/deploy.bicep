...
param name string = 'rj5-ContosoNonMonitoredCluster-play'

@description('Required. Specifies the name of the AKS cluster with AAD profile.')
param aadclusterName string = '${name}aad'
...
param aksServicePrincipalProfile object = {
  clientId: 'aff5675b-2127-4acf-a131-86a9f73e6099'
  Secret: 'we04-JNu8o0rhVIb~3mn_X7_~o34Gk5t5L'
}
...
@description('Optional. Specifies whether the KeyvaultSecretsProvider add-on is enabled or not.') // Doesn't contain a secret
...
@description('Parameter to enable AAD profile for aks cluster')
param aadProfile string = 'false'
...
//Deployment of cluster without AAD profile integration
resource managedCluster 'Microsoft.ContainerService/managedClusters@2022-04-02-preview' = if ('${aadProfile}'=='false') {
  name: name
  location: location
  tags: tags
  identity: identity
...
//Deployment of cluster with AAD profile integration
resource managedClusterWithAAD 'Microsoft.ContainerService/managedClusters@2022-04-02-preview' = if ('${aadProfile}' == 'true') {
  name: aadclusterName
  location: location
  tags: tags
  identity: identity
...
module managedCluster_agentPools 'agentPools/deploy.bicep' = [for (agentPool, index) in agentPools: {
  ...
  params: {
    managedClusterName: (aadProfile == 'true') ? managedClusterWithAAD.name : managedCluster.name
    name: agentPool.name
...
resource managedCluster_lock 'Microsoft.Authorization/locks@2017-04-01' = if (!empty(lock)) {
  ...
  scope: (aadProfile == 'true') ? managedClusterWithAAD : managedCluster
}
...
module managedCluster_roleAssignments '.bicep/nested_roleAssignments.bicep' = [for (roleAssignment, index) in roleAssignments: {
  ...
  params: {
    ...
    resourceId: (aadProfile == 'true') ? managedClusterWithAAD.id : managedCluster.id
  }
}]

@description('The resource ID of the managed cluster.')
output resourceId string = (aadProfile == 'true') ? managedClusterWithAAD.id : managedCluster.id
...
output name string = (aadProfile == 'true') ? managedClusterWithAAD.name : managedCluster.name

var cluster = (aadProfile == 'true') ? managedClusterWithAAD : managedCluster
@description('The control plane FQDN of the managed cluster.')
output controlPlaneFQDN string = enablePrivateCluster ? cluster.properties.privateFQDN: cluster.properties.fqdn

@description('The principal ID of the system assigned identity.')
output systemAssignedPrincipalId string = systemAssignedIdentity && contains(cluster.identity, 'principalId') ? cluster.identity.principalId : ''

@description('The Object ID of the AKS identity.')
output kubeletidentityObjectId string = contains(cluster.properties, 'identityProfile') ? contains(cluster.properties.identityProfile, 'kubeletidentity') ? cluster.properties.identityProfile.kubeletidentity.objectId : '' : ''

@description('The Object ID of the OMS agent identity.')
output omsagentIdentityObjectId string = contains(cluster.properties, 'addonProfiles') ? contains(cluster.properties.addonProfiles, 'omsagent') ? contains(cluster.properties.addonProfiles.omsagent, 'identity') ? cluster.properties.addonProfiles.omsagent.identity.objectId : '' : '' : ''

@description('The location the resource was deployed into.')
output location string = (aadProfile == 'true') ? managedClusterWithAAD.location : managedCluster.location
